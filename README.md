...
Shadow DOM (attachShadow): Used to inject the floating toggle button into the webpage. This ensures the button's CSS (glassmorphism, shaking animations) doesn't break the host website's design, and the website's CSS doesn't break the button.
...
Tailwind CSS: Used for rapid, utility-first styling of the YELL landing page.

GSAP (GreenSock) & ScrollTrigger: Powers the bouncy, scroll-linked animations and horizontal illusion effects on the promotional website.
...
Select your YELL-Extension folder.

Test the Landing Page (Optional):
To view the GSAP-animated promotional website, simply double-click the index.html file to open it in your browser. No local server is required.

## Run

Because this project is built entirely with vanilla JavaScript and native browser APIs, there are zero run commands.

There is no Node.js, no Python server, and no build pipeline. You will never need to type npm install, npm start, or npm run dev in a terminal.

The project "runs" entirely through the browser:

To run the extension: You simply go to chrome://extensions/ and click Load unpacked to load the folder.

To run the landing page: You just double-click index.html to open it directly in Chrome.
